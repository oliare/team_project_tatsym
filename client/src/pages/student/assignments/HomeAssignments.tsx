...
import { getAllHomeworks, getAllSubjects } from '../../../api/homeworks/homeworksApi';
...
const HomeAssignments: React.FC = () => {
  ...
  const [filteredHomeworks, setFilteredHomeworks] = useState<IHomeworkDto[]>([]);
  ...
  useEffect(() => {
    if (selectedSubjectId === '') {
      ...
      const filtered = homeworks.filter((hw) => hw.subjectId === parseInt(selectedSubjectId));
      setFilteredHomeworks(filtered);
    } else {
      setFilteredHomeworks(homeworks); 
    }
  }, [selectedSubjectId, homeworks]);
